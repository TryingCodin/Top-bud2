...
window.addEventListener("DOMContentLoaded", () => {
  ...
  const contactsFormFields = document.querySelectorAll(
    '.contacts-content__form input[type="text"], .contacts-content__form textarea'
  );
  ...
  function addStorageKey() {
    ...
    const menuCatalogNameValue = menuCatalog.textContent
      .replace(/\s+/g, " ")
      .trim();
  ...
  function showProducts(products) {
    ...
    const arr = products.filter((item) => {
      return (
        (names.includes(item[key1]) &&
          selectedValue.includes(item[value1].toString())) ||
        (names.includes(item[key2]) &&
          selectedValue.includes(item[value2].toString()))
      );
    });
    const markup =
      arr.length > 0 ? createProductItems(arr) : createProductItems(products);
    ...
    switch (sortType) {
      case "name-asc":
        return sortedProducts.sort((a, b) =>
          a["Назва_позиції"].localeCompare(b["Назва_позиції"])
        );
      case "name-desc":
        return sortedProducts.sort((a, b) =>
          b["Назва_позиції"].localeCompare(a["Назва_позиції"])
        );
      case "price-asc":
        return sortedProducts.sort(
          (a, b) => parseFloat(a["Ціна"]) - parseFloat(b["Ціна"])
        );
      case "price-desc":
        return sortedProducts.sort(
          (a, b) => parseFloat(b["Ціна"]) - parseFloat(a["Ціна"])
        );
      default:
        return sortedProducts.sort(
          (a, b) =>
            parseFloat(a["Номер_позиції"]) - parseFloat(b["Номер_позиції"])
        );
    }
  }
  ...
  function addActiveClass() {
    const elementsWithDataCategory =
    ...
    const activeCategory =
      categoriesContainer.querySelector(".category_active");
    ...
    if (showList)
      showList.forEach((list) => list.classList.remove("list_show"));
    ...
    elementsWithDataCategory.forEach((elem) => {
      ...
      if (!sessionStorage.getItem("Каталог")) {
        if (
          categoryName === firstStorageKey ||
          categoryName === secondStorageKey
        ) {
          elem.classList.add("category_active");

          if (
            elem.nextElementSibling !== null &&
            elem.nextElementSibling.classList.contains("categories__list-first")
          ) {
            ...
            if (
              elem.nextElementSibling !== null &&
              elem.nextElementSibling.querySelector(".categories__list-second")
            ) {
              elem.nextElementSibling
                .querySelector(".categories__list-second")
                .classList.add("list_show");
            } else if (elem.closest(".categories__list-second") !== null) {
              elem
                .closest(".categories__list-second")
                .classList.add("list_show");
            }
          }
        }
  ...
  function showFilters(products) {
    ...
    getFilterCategories(products).forEach((values, key) => {
      ...
      const listItemsHtml = sortedValues
        .map((value, index) => {
          const hiddenClass = index < 5 ? "expanded" : "";
          const toggle =
            names.includes(key) && selectedValue.includes(value.toString());
        ...
        if (foundItem !== null) {
          ...
          function getCharacteristicValues(filteredProducts) {
            ...
            filteredProducts.forEach((item) => {
              ...
              if (
                characteristicDetail.name1 ||
                characteristicDetail.value1 ||
                characteristicDetail.name2 ||
                characteristicDetail.value2
              ) {
            ...
            sortedGroups.forEach(([name, values]) => {
              ...
              const listItemsHtml = sortedValues
                .map((value) => {
                  const toggle =
                    names.includes(name) &&
                    selectedValue.includes(value.toString());
                  return `<li>
                    <label>
